    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-5 pt-3 sm:px-2 lg:px-4 transition-colors">
            ...
            {warmStatus === 'error' && warmError && (
                <div className="mb-4 text-red-500 text-sm dark:text-red-400">
                    <strong>Warm-up Error:</strong> {warmError}
                </div>
            )}
            {loading && <div className="mb-4 text-gray-500 dark:text-gray-400">Loading model ...</div>}
            {error && <div className="mb-4 text-red-500 dark:text-red-400">Error: {error}</div>}
            {modelData && (
                <>
                    {/* Tabs */}
