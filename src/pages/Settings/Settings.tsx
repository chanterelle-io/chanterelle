          ...
          {/* Appearance / Theme */}
            <div className="mb-8">
              ...
              <fieldset className="space-y-2 mt-6">
                <legend className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Text Size</legend>
                <div className="flex flex-wrap gap-4">
                  {['small', 'medium', 'large'].map(value => (
                    <label key={value} className="inline-flex items-center gap-2 cursor-pointer select-none">
                      <input
                        ...
                        onChange={() => setTextSize(value as any)}
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-slate-600"
                      />
                      <span className="text-sm text-slate-700 dark:text-slate-300 capitalize">{value}</span>
                    </label>
                ...
                <p className="mt-2 text-xs text-gray-500 dark:text-gray-400 max-w-md">Adjust the global text size of the application.</p>
              </fieldset>
            </div>
            {/* Projects directory */}
            <div className="mb-6">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="px-4 py-1.5 bg-green-700 text-white rounded-md hover:bg-green-800 dark:hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors duration-200"
              >
                {loading ? 'Saving...' : 'Save Settings'}
              </button>
              <button
                onClick={() => navigate('/')}
                className="px-4 py-1.5 bg-gray-500 text-white rounded-md hover:bg-gray-600 dark:hover:bg-gray-400 transition-colors duration-200"
              >Cancel</button>
            </div>
        </div>
      </main>
